...
게임을 플레이 중 특정 에셋을 처음 로드하는 경우, LoadObject와 같은 동기 로딩(SyncLoad)을 사용하면 에셋을 즉시 로드할 수 있지만, 로드가 완료될 때까지 게임 스레드는 대기상태가 됩니다.<br/>
따라서 런타임 중에 동기 로드를 사용하게 되면 에셋을 로드할 때 게임이 일시적으로 멈추는 프레임 드랍이 발생할 수 있고, 크기가 큰 에셋이나 여러 에셋을 한꺼번에 로드할 경우에 성능 저하가 발생할 수 있습니다.<br/>
그러므로 동기 로드의 경우 게임을 처음 시작하거나 레벨이 시작되는 시점 등, 필수적으로 대기 시간이 발생하는 시점이나 성능에 영향을 끼치지 않을 정도로 리소스가 작은 에셋을 로드하는 경우에 사용하고,<br/>
유저가 게임을 플레이 하고 있는 시점에 에셋을 로드해야 할 경우에는 비동기 로드(ASyncLoad)를 이용해 게임 플레이 도중 프레임 드랍이나 이미지가 늦게 표시되는 등의 불쾌한 성능저하를 최소화하도록 설계했습니다.
